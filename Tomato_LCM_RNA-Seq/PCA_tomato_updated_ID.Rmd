...
names(data)
head(data)
data$Inter_T <- rowMeans(subset(data[1:4]))
data$C <- rowMeans(subset(data[5:8 & 12:15]))
data$Mature_T <- rowMeans(subset(data[9:11]))
data$Early_T <- rowMeans(subset(data[16:23]))

ave <- subset(data[24:27])
ave$sd <- apply(ave,1,function(d)sd(d))
ave$average <- rowMeans(subset(ave[1:4]))
ave$cv <- ave$sd / ave$average
head(ave)
write.csv(ave,"tomato.LCM.ave.csv")
```

```{r}
## Read in data
data<- read.csv("tomato.LCM.ave.csv",header=TRUE)
data[is.na(data)] <- 0
names(data)
head(data)
attach(data)
...
length(data$X)                           #21159
length(subset(data,Inter_T>0.1)$X)       #20734
length(subset(data,C>0.1)$X)             #21159
length(subset(data,Mature_T>0.1)$X)      #21159
length(subset(data,Early_T>0.1)$X)       #20995
```

```{r}
## Select only those genes upper 60% CV
cv_cutoff <- quantile(data$cv,probs = 0.6)
cv_cutoff
sub.data <- subset(data, cv > cv_cutoff)
head(sub.data)
length(sub.data$X) #10579
```

```{r}
## Create a matrix of the data to perform a PCA on and scale it
#library(clusterSim)
m.sub.data <- as.matrix(sub.data[2:5])
head(m.sub.data)
...
load <- load[1:4]
load$sample <- c("Inter","C","Mature","Early")
